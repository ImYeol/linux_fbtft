...
void fbtft_set_addr_win(struct fbtft_par *par, int xs, int ys, int xe, int ye)
{
	...
	printk("fbtft_set_addr_win\n");
	write_reg(par,0x33,0x00,0x00,(VSA>>8) & 0xFF,VSA & 0xFF,0x00,0x00);
	...
	write_reg(par,0x37,(VSP >> 8)&0xFF,VSP&0xFF);
...
//		__func__, start_line, end_line);
	...
	printk("%s(start_line=%u,start_line_t=%u,end_line=%u)\n",__func__,start_line,start_line_t,end_line);
...
void fbtft_fb_copyarea(struct fb_info *info, const struct fb_copyarea *area)
{
	...
	printk("copyare :x-%d ,y-%d,width-%d,height-%d\n",area->dx,area->dy,area->width,area->height);
